...
if(count(array_intersect_key($_POST, array_flip(array('submitted', 'username', 'password')))) == 3)
{
	...
	$params = array(array('query_key' => 0, 
						  'params' => array('username' => $user, 
											'password' => $password)));
	$ds_mkt->execQuery(array($query), TRUE, $params);
	while($ds_mkt->result != FALSE && $data = $ds_mkt->fetchArray())
	{
...
if(isset($_SESSION['marketing_user']) && count($_SESSION['permissions']) > 0)
{
...
{	
	...
	$ds_mkt->execQuery($query, TRUE, $params);
}
#-----------------------------------------------------------------------------------------------------------#
